...
const ormSpecies = require('../orm/orm-Species');
...
exports.GetAll = async(req, res) => {
    ...
    let response = {};
    try {
        let respOrm = await ormSpecies.GetAll();
        if(respOrm.error) {
            (status = "Failure"),
            ...
            (message = "Succes GetAll species"), (data = respOrm), 
...
exports.Create = async (req, res) => {
    ...
    try {
        ...
        if (Name, Reproduction, Class, Order, Species) {
            let respOrm = await ormSpecies.Create(Name, Reproduction, Class, Order, Species);
            if (respOrm.error) {
                (status = 'Failure'), (errorcode = respOrm.error.code),
                (message = respOrm.error.message), (statuscode = enum_.CODE_BAD_REQUEST);
            } else {
                (message = 'Species created'), (statuscode = enum_.CODE_CREATED);
            }
        } else {
            (status = 'Failure'),
